<x-layouts.app title="Create User">
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-bold text-gray-900">{{ isset($isTeamLeader) && $isTeamLeader ? 'Create Field Force User' : 'Create User' }}</h1>
    </div>
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <form method="POST" action="{{ route('users.store') }}" class="space-y-6">
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('users.index') }}" class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">Cancel</a>
                <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700">Create User</button>
            </div>
        </form>
    </div>
</x-layouts.app>
