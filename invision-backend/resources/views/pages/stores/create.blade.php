<x-layouts.app title="Add Store">
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-600">Create a new store in the database.</p>
    </div>
    ...
    <form method="POST" action="{{ route('stores.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="rank" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        @foreach(App\Enums\StoreRank::cases() as $rank)
                            <option value="{{ $rank->value }}" {{ old('rank', 'unranked') === $rank->value ? 'selected' : '' }}>{{ $rank->label() }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
        ...
        <div class="bg-white shadow rounded-lg p-6">
            ...
            <p class="text-sm text-gray-500 mb-4">Click on the map to set the store's GPS coordinates, or drag the marker to adjust.</p>
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('stores.index') }}" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-md text-sm hover:bg-gray-50">Cancel</a>
            <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm hover:bg-indigo-700">Create Store</button>
        </div>
    </form>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const map = L.map('store-map').setView([24.7136, 46.6753], 10);
